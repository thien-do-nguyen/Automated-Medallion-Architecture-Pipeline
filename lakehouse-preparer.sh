...
echo "Creating schema/tables in Trino..."
docker compose exec trino trino --file /opt/trino/iceberg-schema-gold.sql

echo "Transforming silver to gold tables (Feature Engineering)..."
docker compose exec spark-iceberg /opt/spark/bin/spark-submit \
  /home/iceberg/pyspark/scripts/compute_features.py

echo "Training ALS Recommendation Model..."
docker compose exec spark-iceberg /opt/spark/bin/spark-submit \
  --jars /home/iceberg/pyspark/jars/postgresql-42.7.6.jar \
